...
# Definir qual camera será utilizada na captura (webcam)
#camera = cv2.VideoCapture(0)

# Carregar vídeo

# camera = cv2.VideoCapture('input/experiment/highway_edited.mp4')
# camera = cv2.VideoCapture('input/experiment/mall_edited.mp4')
# camera = cv2.VideoCapture('input/experiment/school_edited.mp4')
camera = cv2.VideoCapture('input/experiment/street_edited.mp4')


# Using external video and writing output
fourcc = cv2.VideoWriter_fourcc('X','V','I','D')
# out = cv2.VideoWriter("output/highway.mp4", fourcc, 5.0, (1280,720))
...
# Definir limite para filtrar caixas delimitadoras fracas com supressão não máxima
threshold = 0.3
...
with open('output/csv/street.csv', 'w') as arquivo:
    ...
    while True:
        ...
        if len(results) > 0:
            for i in results.flatten():
                ...
                pct_diff = 1.0 - confidences[i]
                red_color = min(255, pct_diff*2 * 255)
                green_color = min(255, confidences[i]*2 * 255)
                colour_box_current = (red_color, green_color, 0)
        ...
        print('{:.5f} segundos'.format(time.time() - ex_start))
...
# tempo gasto 
print('Tempo gasto video : {:.5f} segundos'.format(ex_end - ex_start))

camera.release()
cv2.destroyAllWindows()


# usar ret do read como criterio de parada olhar codigo maria